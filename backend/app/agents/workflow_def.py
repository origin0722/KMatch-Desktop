...
BUILTIN_WORKFLOWS: dict[str, dict] = {
    ...
    "scene2-project": {
        ...
        "id": "scene2-project",
        "name": "场景二·项目二次开发",
        "description": "代码解析→项目图谱→代码审查→代码测试(打回再生)→修复指引 (W6 起有真实执行器 project_workflow, parse/图谱由打开项目时完成)",
        "stages": [
            ...
            {"id": "repair", "label": "修复指引", "agents": ["content_generator"], "dependencies": ["code-test"]},
        ],
        "inputs": {"required": ["target_direction"], "schema": {"target_direction": "string", "scene": "string"}},
    },
}
