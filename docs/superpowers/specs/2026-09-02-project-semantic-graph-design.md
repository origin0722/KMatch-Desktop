...
> 日期：2026-09-02  
> 关联：[画像协同历史与体验升级总方案](../../项目规划/2026-09-02_画像协同历史与体验升级总方案.md) · [项目语义图谱框架图](../../架构与设计/项目语义图谱_框架图.html)
...
> **实现状态（2026-09-02）**：本轮 P0 只修复了当前 Python 文件质量检查的真实命名、画像读取和能力边界，并补足项目/学习图谱历史的安全单项移除。Java/Spring Boot、TypeScript、Rust、C 的结构或框架图谱均**尚未实现**；Spring Boot 仍是下文定义的下一条完整纵切，不能在产品界面或答辩中宣称已支持。
...
- 不直接执行未知项目代码；
- 不自动修改用户源码；
- 不把项目源码、README、注释或配置中的文本当作 Agent 指令；
- 不跟随指向项目根目录之外的符号链接读取文件；
...
“完整框架图谱”在本设计中的含义是：完整覆盖一个**已声明的框架 profile**中的角色、关系、配置和质量证据清单；它不表示可还原框架所有运行时反射、动态代理、宏展开或第三方库内部行为。
...
```json
{
  ...
  "adapter_versions": {"java": "1.0", "spring-boot": "1.0"}
}
```
...
- 不跟随项目根目录外的符号链接；解析器和 Agent 的可见路径必须经过同一根目录守卫；
- 首次扫描只读取元数据和允许的文本文件；
- 展示“将读取的文件数、语言、大小、忽略规则”；
- 允许用户将范围缩小为模块或变更文件；
- 不执行构建命令、脚本、安装命令或项目源码。

所有项目文件都属于不可信输入：在送入框架理解 Agent 前，移除或截断敏感配置值，保留键名、类型、结构、路径和内容 hash；README、注释、代码字符串中的“指令”只能作为项目内容解释，不能改变 Agent 权限、范围或执行策略。
...
混合语言项目的跨语言边界只在存在明确证据时建立：REST/OpenAPI 路由、消息 topic、数据库 schema、FFI 声明、构建模块依赖或受识别配置。类名、函数名或目录名相同不能自动形成跨语言调用边。
...
| 测试工具不可用或用户未确认 | 保存静态图谱、跳过动态质量结果 | “未运行测试：缺少工具、沙箱能力或执行授权” |
...
- 包含恶意 README、注释、`.env`、符号链接和超大文件的项目：扫描范围不被篡改，敏感值不进入 Agent 摘要；
- 未确认测试执行：绝不启动 Maven/Gradle/npm/Cargo 或项目脚本；确认后也只在隔离沙箱和限定网络/时间资源中运行；
...
| 项目文本诱导 Agent 越权 | 所有项目内容按不可信数据处理，权限只由产品策略和用户确认决定 |
| 符号链接或配置泄露根目录外文件 | 根目录守卫、链接拒绝、敏感值脱敏和读取清单预览 |
