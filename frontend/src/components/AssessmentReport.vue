  ...
  <div class="assessment-report">
    ...
    <div class="summary-bar">
      <div class="summary-stat">
        ...
        <div class="stat-label">正确 / 总题数</div>
      </div>
      <el-progress
 ...
 *
 * 三态判分 (BUG-019): null=未评分 / true=正确 / false=错误
 * nodeCorrect 用 === true 而非 filter(Boolean) (BUG-029: grade 是对象, truthy 会全计入)
 */
...
// --- 颜色 (el-progress :color 用 hex 镜像 --km-* token) ---
const THEME = {
  good: '#34b37e',
  ok: '#f0a040',
  bad: '#e05555',
...
const accuracyColor = computed(() => {
  if (accuracy.value >= 0.8) return THEME.good
  if (accuracy.value >= 0.5) return THEME.ok
  return THEME.bad
})
...
// per_node 结构: [{question_index, correct}, ...]
...
const questionList = computed(() => {
  ...
  return questions.map((q, idx) => ({
    ...
    options: q.options,
    answer: answers[idx] ?? null,  // 保留 null 让模板 ?? 兜底生效（'' 不触发 ??）
    grade: { correct: questionGrades.value[idx]?.correct ?? null },
  }))
})

// --- 工具 ---
const TYPE_LABELS = { choice: '选择题', judge: '判断题', code: '代码题', fill: '填空题' }
function typeLabel(type) {
  return TYPE_LABELS[type] ?? type ?? '未知'
}

// 三态判断: null=未评分, true=正确, false=错误
