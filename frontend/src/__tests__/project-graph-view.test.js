...
vi.mock('@/stores/graphHistory', () => ({
  useGraphHistoryStore: () => ({ items: [], viewLearning: vi.fn(), backToLiveLearning: vi.fn() }),
}))
...
vi.mock('element-plus', () => ({
  ...
  ElMessageBox: { prompt: mocks.prompt, confirm: vi.fn() },
}))
...
const ElButton = {
  inheritAttrs: false,
  template: '<button :data-test="$attrs[\'data-test\']" :disabled="$attrs.disabled" @click="$emit(\'click\')"><slot /></button>',
}
const stubs = {
  ElAlert: true,
