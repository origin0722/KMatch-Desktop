...
> 赛题 XH-202630 ｜ 单人全栈 ｜ **正式版 v1.1.0** ｜ Windows 桌面 IDE
...
> 📦 正式版发行物：`KMatch-Desktop-1.1.0-x64.exe` — [GitHub Release v1.1.0](https://github.com/origin0722/KMatch-Desktop/releases/tag/v1.1.0)
> 📖 详尽功能说明：[docs/交付材料/软件说明_v1.1.0.md](docs/交付材料/软件说明_v1.1.0.md)
...
| 学习引擎 | 学情测评 → 个性化路径（BFS+掌握度+折周估时）→ 分层讲义/实操指南/分阶测试题 → 四维审核打回再生 → 动态迭代（降维/进阶）→ 画像跨次进化 |
| 知识图谱 | 四层图谱（6 域 222 节点 + 93 题库）+ 动态建域 + 路径查找 + 历史图谱快照 |
| 项目图谱 | 打开项目自动解析 + Monaco 符号联动 + 架构解读 + 代码审查/测试双 Agent |
| Git 仓库 | 克隆远程（自动切换项目）/ 初始化 / 状态着色列表 / 暂存提交 / 拉取推送 / 最近提交 / 文件变动实时刷新（无需终端） |
| 运行历史 | 结构化 run 事件落盘 + 复盘 + 一键重跑 / 重新测评 + 流程图 DAG |
| 设置页 | AI 助手 / 学习引擎（Key 自动回退 AI 助手）/ 联网搜索（一键测试）/ 自定义厂商 / 网络代理 / 主题皮肤（**默认深青珊瑚**） |
...
产物 `release/KMatch·知链-1.1.0-x64.exe`，装后 sidecar 自启；**端用户免 Docker**。首次打包需开 Windows 开发者模式（winCodeSign 符号链接）。详见 [CLAUDE.md](CLAUDE.md)。
...
│   └── __tests__/     # Vitest 单测（467 用例）
├── backend/app/       # FastAPI + LangGraph（agents/ graph/ api/）+ tests（690 通过 / 2 跳过）
...
- **测试**：前端 **467 用例全过**（Vitest）；后端 **690 通过 / 2 跳过**（Pytest）。
- **M5 指标**：幻觉率 <5% / 适配率 ≥85% / 覆盖率 ≥90%（独立裁判双口径，3 组画像实测达标）。
- **赛题要求**：≥3 个分工明确 Agent（实际 7+2）、分析-生成-校验-决策闭环、先验画像+专业知识库融合、
  三形态个性化资源、可视化学情报告、动态迭代机制、多 Agent 辩论抗幻觉、数据合规、场景二延伸——自查表见
...
> 完整文档导航见 [docs/README.md](docs/README.md)；详尽功能说明见 [docs/交付材料/软件说明_v1.1.0.md](docs/交付材料/软件说明_v1.1.0.md)。
