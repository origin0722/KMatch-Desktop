...
同日另有：提示词-代码契约漂移测试（`test_prompt_contract`）、设置页「API 设置」栏目（`apiSettings`：AI 助手与出题引擎 API 统一/分开 + 预设模型 + 连通性测试支持 openai/anthropic）、分阶测试题先自测后对答案、知识图谱详情「重测该点」/「问 AI 助手」双入口、Agent 协同降级(`degraded`)可视化。动态建域 PU 迷你域入库（292 节点/648 题/11 画像）。测试：前端 409 / 后端 58 全过。devlogs 见 [docs/devlogs/B_前端/](docs/devlogs/B_前端/)。
