...
- 安装包 `release/KMatch·知链-1.3.4-x64.exe` 可直接分发, 装后 sidecar (KMatchBackend.exe) 自启; **端用户免 Docker** —— 安装包默认进程内嵌入式存储 (GRAPH_STORE=embedded, `backend/app/graph/embedded.py`), Neo4j 仅为可选 dev/演示后端 (GRAPH_STORE=neo4j)。
- 版本迭代: #59/#60 已落地: 随包便携 Python 运行时(安装包内代码测试可用) + Electron sandbox/CSP 加固。v1.0.3 判分/再生/补跑预检应用 llm_overrides (1361d5f); v1.0.4 判分"卡死"(超时放宽+等待计时+错误条); v1.0.5 HTTP 0/未知错误 真实错误上浮 + axios 解构误用横扫; v1.0.6 判分专属 45s/0 重试快失败 + submit 步骤耗时日志; v1.0.7 判分/反馈 async 防线程池排队 + 状态写死锁修复 + API 连通性扫描备忘; v1.0.8 AI 助手只读工具结果回喂 + 学情答案解析 + 讲义生成入口 + 图谱自适应 + 画像知识点名称补齐; v1.0.9 用户审阅 10 项整改 (导航折叠/启动就绪门/设置收敛三大块/小字清理/学习目标持久化/运行历史场景与薄弱点/题库审计与内容深度/薄弱点联网工具/时间估计修正/图谱导读对话化); v1.0.11 第二批 12 项整改 (深度透明导航/菜单移除/设置精简+新选项/快捷扩展/思考截断/详情收起/导读定位/时间校准/文件格式/非Py提示/多面板/撞色) + 运行历史删除/空态居中/无边框窗口/模型升代/对话样式QA; v1.0.12 小字清理/最近打开删除/空态居中+功能说明/图谱节点放大; v1.0.13 认证回退(AI助手key兜底)/移除.env指引/Tavily测试连接/图谱开关清空+历史/对话输入固定+流式指标/Git仓库面板(克隆·提交·拉取·推送,issue-93); v1.1.0 正式版(默认皮肤深青珊瑚/Git实时刷新/全量终审467+690测试/软件说明+README更新); v1.2.0 (设置中心补齐+运行时配置层+配置导入导出/场景二LangGraph编排/VARK三维测评/图谱CJK布局+excalidraw导出/学习资源扩量5节点/AI助手四项优化/全量726+507测试); 2026-09-01 赛题对标加固 (独立Key粘贴401修复/图谱历史只读回看+返回导航/难度匹配真曲线/学历背景+VARK接入生成prompt/申诉-复审辩论制/裁判接入在线报告, 746+524测试)。v1.3.0 (截断治理/API 自愈/画像细致化/交互日志脱敏/测试数据 I/O 示例策展); v1.3.1 (动态建域出题线程 overrides 传播修复——JAVAEE 等未收录领域安装包必现 401 的根因, 777+538 测试)。方案见 [docs/架构与设计/轻量化改造方案_免Docker_嵌入式存储.md](docs/架构与设计/轻量化改造方案_免Docker_嵌入式存储.md)。v1.3.2 (P0 项目图谱可信度: 当前文件质量检查入口修复+诚实命名/运行历史展示投影不泄漏 request/图谱历史单项移除/资源阶段延后启动如实表达/测试环境隔离; 预览版用户反馈收口: 未开项目恢复旧图谱横幅+清除入口/返回键无备份可退出回看/移除回看中快照联动退出/反馈再生 5min 放宽+截止有界收集不再整单取消/AssistantSidePanel 投影字段对齐/流程 DAG deferred 视觉, 780+549 测试)。v1.3.3 (体验跃升五线: 等待优化感知层——interactive submit/feedback/report 三端点 SSE 流式+逐资源进度+停止等待+等待文案真实化; 真实提速——独立裁判并行化+审核打回节点级再生缓存+判分可选快模型分流+全链耗时打点; 图谱——stale 检测失效修复(防按旧行号跳错)+解析提速四点+prereq 批量 API 消 N+1+KG 高亮增量化+PG 搜索防抖+边类型过滤图例+大项目默认模块聚合+动态领域补色板+搜索降级提示+路径上图+query_project_graph 上下文裁剪; 去AI味——00 共享契约新增文风契约(test 资源格式豁免冻结)+机器标记出正文+MarkdownViewer 净化与流式防跳变+chat 回答格式规范+UI 文案/emoji 清理约 25 处, 794+564 测试)。v1.3.4 (热修: 流式提交载荷携带 Vue 响应式代理过 IPC 结构化克隆报 "An object could not be cloned." 致桌面端提交答题/反馈全挂 — postSseJson 单点 JSON round-trip 转纯对象 + 3 条 IPC 可克隆回归测试; 797+569 测试)。
...
**最新 (2026/08/19)：端用户免 Docker — 嵌入式图存储 (v1.0.0)**（方案 [轻量化改造方案_免Docker_嵌入式存储.md](docs/架构与设计/轻量化改造方案_免Docker_嵌入式存储.md)，ADR-0008，issue #50-56）：新增 `EmbeddedGraphStore`（`backend/app/graph/embedded.py`，~700 行）+ 14 个嵌入式单测。`GRAPH_STORE=neo4j|embedded|auto`——安装包 frozen 态默认 `embedded`（run_server 兜底）、dev/测试默认 `neo4j`（保现有集成测试零漂移）。嵌入式以 `data/knowledge_base` JSON 为真相源 + `data/local/` 落盘可变数据（项目图谱/掌握状态/风险/向量），查询零网络往返（BFS 等价变长 Cypher），向量语义本地缓存 + 后台自动回填 + 降级纯图（语义守卫切 `semantic_ready`，唯一 1 行契约改动在 `graph.py:108`）。前端 StatusBar 增「本地存储 / 纯图模式」状态点，Docker 引导弹层嵌入式下自动隐藏。后端 **657 测(+2 skip)** + 前端 **434 测**全过。同日跟进：`KMATCH_LOCAL_DIR`（安装包可写用户目录，config/embedded/sidecar 接线）、离线种子导出 `scripts/export_embeddings.py`、真实数据回归+等价性胶水 `tests/test_engine_parity.py`、PyInstaller sidecar 无 Docker 冒烟 **8/8 PASS**。devlog 见 [docs/devlogs/A_后端/2026-08-19_端用户免Docker_嵌入式存储.md](docs/devlogs/A_后端/2026-08-19_端用户免Docker_嵌入式存储.md)。
