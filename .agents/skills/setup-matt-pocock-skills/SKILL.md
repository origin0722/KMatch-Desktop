...
- repo 根目录的 `AGENTS.md` 和 `AGENTS.md` — 是否存在？其中是否已经有 `## Agent skills` section？
...
- 要添加到 `AGENTS.md` / `AGENTS.md` 中的 `## Agent skills` block（选择规则见 step 4）
...
当 `AGENTS.md` 已经存在时，绝不创建 `AGENTS.md`（反之亦然）；始终编辑已经存在的那个。
